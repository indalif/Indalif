...
document.getElementById('notaPedidoForm').addEventListener('submit', async function (event) {
    ...
    const idNota = document.getElementById('notaPedidoForm').dataset.idNota || null;
    ...
    try {
        ...
        const url = idNota ? `/notas-pedido/${idNota}` : '/notas-pedido';
        ...
        alert(idNota ? 'Nota de pedido actualizada con éxito!' : 'Nota de pedido guardada con éxito!');
        document.getElementById('notaPedidoForm').reset();
        document.getElementById('notaPedidoForm').removeAttribute('data-id-nota');
        productosLista = [];
...
async function cargarNotas() {
    try {
        ...
        data.forEach(nota => {
            ...
            let productosHTML = nota.productos.map(p => `<li>${p.producto} - Cantidad: ${p.cantidad}, Presentación: ${p.presentacion}</li>`).join('');
            ...
            div.innerHTML = `
                ...
                <h6>Productos:</h6>
                <ul>${productosHTML}</ul>
                <button onclick="editarNota(${nota.id})" class="btn btn-warning w-100 mt-2">Editar</button>
            `;
...
async function editarNota(id) {
    try {
        ...
        document.getElementById('notaPedidoForm').dataset.idNota = id;
    } catch (error) {
